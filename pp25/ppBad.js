        ...
        if (status !== 'success') {
            ...
            window.setTimeout(function () {
                var zan = page.evaluate(function () {
                    return  document.querySelectorAll('span[class="poor-ico voteTarget"]')[2].getBoundingClientRect();//远洋妙医
                })
            ...
            window.setTimeout(function () {
                console.log('第二步完成 ...');
                page.render("./images/"+ new Date().getDay()+','+new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds()+"yuanyangBad.png");
                //page.render('zan1.png')
                //var zan = page.evaluate(function () {
                //    return  document.querySelectorAll('span[class="good-ico voteTarget"]')[2].getBoundingClientRect();//远洋妙医
                //})
                var miaoZan = page.evaluate(function () {
                    return  document.querySelectorAll('span[class="poor-ico voteTarget"]')[0].getBoundingClientRect();//妙医
                })
                page.sendEvent('mousemove', miaoZan.left + miaoZan.width/2 , miaoZan.top + miaoZan.height/2);
                page.sendEvent('click', miaoZan.left + miaoZan.width/2 , miaoZan.top + miaoZan.height/2);
                //page.sendEvent('mousemove', zan.left + zan.width/2 , zan.top + zan.height/2);
            ...
            window.setTimeout(function () {
                console.log('第三步完成 ...');
                page.render("./images/"+ new Date().getDay()+','+new Date().getHours()+':'+new Date().getMinutes()+':'+new Date().getSeconds()+"bad.png");
                cb(null,'');
            }, 20000)
        }
    })
